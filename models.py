...
def create_modules(module_defs, img_size):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        # 跨层连接
            ...
            modules.add_module(
                'cbam',
                BasicBlock(output_filters[-1], output_filters[-1])
            )
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers):
        ...
        self.na = len(anchors)  # number of anchors (3) 该YOLOLayer分配给每个grid的anchor的个数
        self.nc = nc  # number of classes (80) 类别个数
        self.no = nc + 5  # number of outputs (85) 每个格子对应输出的维度
