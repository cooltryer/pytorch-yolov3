...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, root_dir, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_labels=True, cache_images=False, single_cls=False):
        print("path", path)
        path = str(Path(path))  # os-agnostic  data_ship/train.txt
        assert os.path.isfile(path), 'File not found %s. See %s' % (path, help_url)
        with open(path, 'r') as f:
            self.img_files = [os.path.join(root_dir, "Images/" + x + ".jpg") for x in f.read().splitlines()]

        n = len(self.img_files)  # 一共有多少张图片（train和test不一样）
        assert n > 0, 'No images found in %s. See %s' % (path, help_url)
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # batch index 类似打标签吧
        ...
        # data_ship/images/1833.jpg  这是x的值，目的是将images替换成labels获取到labels文件夹下面的txt文件信息。
        with open(path, 'r') as f:
            self.label_files = [os.path.join(root_dir, "labels/" + x + ".txt") for x  in f.read().splitlines()]

        # Rectangular Training  https://github.com/ultralytics/yolov3/issues/232
        if self.rect:
            # Read image shapes (wh)
            sp = path.replace('.txt', '.shapes')  # shapefile path  data_ship/test.shapes
            try:
                with open(sp, 'r') as f:  # read existing shapefile
                    s = [x.split() for x in f.read().splitlines()]  # 得到image的尺寸，（在test.shapes里面写的）
            ...
            s = np.array(s, dtype=np.float64)  # 转成numpy格式  数据记录格式（1024, 600）（长，宽）
            ar = s[:, 1] / s[:, 0]  # aspect ratio （宽/长的比例）
            i = ar.argsort()   # argsort()返回数组中从小到大的索引值。
            self.img_files = [self.img_files[j] for j in i]  # self.img_files 存储每个图像的地址列表
            self.label_files = [self.label_files[j] for j in i]  # self.label_files 存储label的地址
            self.shapes = s[i]  # wh
            ar = ar[i]  # 按照顺序重新排序好
        ...
        if cache_labels or image_weights:  # cache labels for faster training
            # 5个标签值分别为。图的类别编号（检测为0）；归一化后的中心点x坐标；归一化后的中心点y坐标；归一化后的目标框宽度；归一化后的目标框高度
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            # Load image
            # 加载图片（h0，w0）原始尺寸，（h，w）变换后的尺寸
            img, (h0, w0), (h, w) = load_image(self, index)

            # Letterbox
            # 仿射变换
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)  # pad:(w, h)
            shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling

            # Load labels
            # 加载标注文件
            labels = []
            if os.path.isfile(label_path):  # 判断label_path是否为文件
                x = self.labels[index]
                if x is None:  # labels not preloaded  如果label没有加载，读取label_path的内容
        ...
        # Convert
        # 图像维度变换，编程RGB ，因为opencv 读取的图像格式是BGR
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
