...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    tcls, tbox, indices, anchor_vec = build_targets(model, targets)  #在13 26 52维度中找到大于iou阈值最适合的anchor box 作为targets
    ...
    # p = [[bs, 3, 13, 13, 85], [bs, 3, 26, 26, 85], [bs, 3, 52, 52, 85]].
    for i, pi in enumerate(p):  # layer index, layer predictions
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx 从返回的标签中读取最佳anchor，与格子坐标，每个ground truth只对应一个anchor box，这些索引就是为了找到那个anchor box。
        ...
        if nb:  # number of targets
            ng += nb
            ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets 找到pi中对应的 ground truth的数据
...
def build_targets(model, targets):
    ...
    for i in model.yolo_layers:  # 有三个yolo层，i是层的索引
        # get number of grid points and anchor vec for this yolo layer
        # anchor_vec是anchor box的wh， ng是网格点数
        if multi_gpu:
        ...
        t, a = targets, []
        gwh = t[:, 4:6] * ng  # t[:, 4:6]：ground truth的宽和高，gwh将原来的wh还原到特征图上
        if nt:
            # 求出每一层三个anchor box与ground truth的iou，然后找到iou最大的anchor box
            # 用a来记录它的索引，删去小于iou_thres的anchor。用b记录imges，c记录class类别，gxy记录对应feature map的中心点，
            # gi，gj记录哪个格子负责这个ground truth。
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if method == 'merge':  # Merge NMS (boxes merged using weighted mean)
            # 手写
            b_boxes = boxes.cpu().detach().numpy()
            b_scores = scores.cpu().detach().numpy()
            b_scores = b_scores[:, np.newaxis]
            hanxu = np.hstack((b_boxes, b_scores))
            i = py_cpu_nms(hanxu, iou_thres)
            i = torch.LongTensor(i)
            # i = torchvision.ops.boxes.nms(boxes, scores, iou_thres)
            if n < 1000:  # update boxes
                iou = box_iou(boxes, boxes).tril_()  # lower triangular iou matrix
                hhh = iou > iou_thres
                hhh__ = torch.from_numpy(hhh.data.cpu().numpy() + 0).float().cuda()
                weights = hhh__ * scores.view(-1, 1)
                # weights = (iou > iou_thres) * scores.view(-1, 1)
                weights /= weights.sum(0)
                x[:, :4] = torch.mm(torch.t(weights), x[:, :4])  # merged_boxes(n,4) = weights(n,n) * boxes(n,4)
        elif method == 'vision':
            i = torchvision.ops.boxes.nms(boxes, scores, iou_thres)
